...
const CustomizeProducts = ({ productId, variants, productOptions }: { productId: string; variants: products.Variant[]; productOptions: products.ProductOption[] }) => {
    ...
    const isVariantInStock = (choices: { [key: string]: string }) => {
        return variants.some((variant) => {
            ...
            return (
                Object.entries(choices).every(([key, value]) => variantChoices[key] === value) && variant.stock?.inStock
            )
        })
    }
    ...
    return (
        <div className='flex flex-col gap-6'>
                    ...
                    <ul className='flex items-center gap-3'>
                        {option.choices?.map((choice) => {
                            ...
                            return option.name === "Color" ? (
                                <li key={choice.description} onClick={clickHandler} className='w-8 h-8 rounded-full ring-1 ring-gray-300 relative' style={{ backgroundColor: choice.value, cursor: disabled ? "not-allowed" : "pointer" }}>
                                ...
                                <li key={choice.description} onClick={clickHandler}  className='ring-1 ring-cartColor text-cartColor rounded-md py-1 px-4 text-sm' style={{cursor: disabled ? "not-allowed" : "pointer", backgroundColor: selected ? "#f35c7a" : disabled? "#FBCFE8" : "white", color: selected || disabled ? "white" : "#f35c7a", boxShadow: disabled? "none": "" }}>{choice.description}</li>
                            )
                        })}
                    </ul>
                </div>
            ...
            <ul className='flex items-center gap-3'>
                <li className='ring-1 ring-cartColor text-cartColor rounded-md py-1 px-4 text-sm cursor-pointer'>Small</li>
                <li className='ring-1 ring-cartColor text-white bg-cartColor rounded-md py-1 px-4 text-sm cursor-pointer'>Medium</li>
